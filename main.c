...
int main(void)
{
  ...
  MX_I2C1_Init();
  /* USER CODE BEGIN 2 */
  // Bring up the accelerometer
  if (adxl_init() != HAL_OK) {
    // hard error: slow blink forever
    while (1){ HAL_GPIO_TogglePin(GPIOB, GPIO_PIN_5); HAL_Delay(500); }
  }

  // Calibrate (keep the board still!) and blink LED 3× when done
  if (adxl_calibrate() != HAL_OK) {
    while (1){ HAL_GPIO_TogglePin(GPIOB, GPIO_PIN_5); HAL_Delay(300); } // calib error
  }
  led_blink_times(3, 120, 120);   // calibration finished
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  now = HAL_GetTick();
	  if ((int32_t)(now - next_read) < 0) {
	    // keep a steady sample period
	    continue;
	  }
	  next_read += READ_PERIOD_MS;

	  // ----------- 1. Read sensor -----------
	  int16_t rx, ry, rz;
	  if (adxl_read_xyz(&rx,&ry,&rz) != HAL_OK) {
	    // read error: quick blink so you notice, then keep going
	  ...
	  // ----------- 5. Trigger / blink logic -----------
	  // If shaking, extend the "hold" window forward by 10 s
	  if ((st - lt) > TRIGGER_DELTA_G) {
	    hold_until = now + LED_POST_HOLD_MS;
	  }

	  // Blink LED while in hold window
	  if ((int32_t)(hold_until - now) > 0) {
	    // Blink non-blocking every BLINK_PERIOD_MS
	    if ((int32_t)(now - next_blink_toggle) >= 0) {
	      led_state ^= 1;
	      HAL_GPIO_WritePin(GPIOB, GPIO_PIN_5,
	                        led_state ? GPIO_PIN_SET : GPIO_PIN_RESET);
	      next_blink_toggle = now + BLINK_PERIOD_MS;
	    }
	  } else {
	    // Quiet: LED off
	    if (led_state) {
